...
"try { $listener.Start(); } catch { Write-Host 'Port 8080 is in use. Close other apps and try again.'; Start-Sleep -s 5; exit; }" ^
...
"        elseif ($filePath -match '\.mp3$') { $response.ContentType = 'audio/mpeg' }" ^
"        elseif ($filePath -match '\.wav$') { $response.ContentType = 'audio/wav' }" ^
